...
This plugin supports all 256 documented AIDP CLI commands through `aidp_cli`, with lookup support through `aidp_cli_reference`.

## Agent (agent)
Agents, agent deployments, agent sessions, and agent permissions.

- `aidp agent copy` - Copies an agent.
- `aidp agent create` - Creates an agent.
- `aidp agent delete` - Delete an agent from the schema.
- `aidp agent delete-deployment` - Deletes an agent deployment.
- `aidp agent delete-session` - Deletes an agent Session.
- `aidp agent deploy` - Deploys a specified agent.
...
- `aidp agent list-deployments` - Returns a list of all deployments of an agent.
...
- `aidp agent update` - Updates an agent with provided details.
- `aidp agent update-deployment-metadata` - Updates the deployment metadata for an agent.
- `aidp agent validate-agent` - Validates the agent JSON diagram generated by UI.
...
- `aidp bundle create` - (Deprecated). (Deprecated)
- `aidp bundle create-action` - (Preview) Creates a new bundle. (Preview) Creates a new bundle. This operation is asynchronous. The service validates the request, starts bundle creation, and returns an async operation key in the response headers. Use the async operation APIs to track completion. Typical use cases: - capture selected workspace resources into a version-controlled bundle - prepare a bundle for later deployment or promotion - establish a bundle root that can later be inspected, updated, or deployed Request notes: - path identifies the parent folder in the workspace volume where the bundle should be created - nam
- `aidp bundle deploy` - (Deprecated). (Deprecated)
- `aidp bundle deploy-action` - Deprecated compatibility API. Deprecated compatibility API. Use publish for new callers.
- `aidp bundle fetch-deployment-status` - (Deprecated). (Deprecated)
- `aidp bundle fetch-deployment-status-action` - Deprecated compatibility API. Deprecated compatibility API. Use getBundlePublishStatus for new callers.
- `aidp bundle fetch-publish-status-action` - (Preview) Returns the latest publish summary.
- `aidp bundle publish-bundle-action` - (Preview) Publishes the specified bundle, creating or updating jobs and agent flows according to the bundle manifest. (Preview) Publishes the specified bundle, creating or updating jobs and agent flows according to the bundle manifest. Returns an async job key for tracking publish progress. This operation is asynchronous. The request is accepted for background execution and returns an async operation key in the response headers. Publishing typically uses: - the bundle manifest at the bundle root - top-level resource descriptors in the bundle - dependency descriptors referenced by those top-lev
- `aidp bundle purge` - (Deprecated). (Deprecated)
...
- `aidp cluster clone-compute` - Creates one Spark Compute by copying all source Compute settings and configuration.
...
- `aidp cluster export-compute-configuration` - Writes selected Compute configuration values supplied by the caller to a workspace YAML file without overwriting an existing file.
- `aidp cluster get` - Returns detailed information about a cluster.
- `aidp cluster get-compute-configuration` - Gets cluster-scoped Python and JAR libraries and environment variables from Spark Compute.
- `aidp cluster get-default` - Gets information about the master catalog default cluster.
- `aidp cluster import-compute-configuration` - Imports one or more unique workspace YAML files into an active Spark Compute.
...
- `aidp cluster search-maven-packages` - Searches Maven packages available for cluster library installation.
...
- `aidp data-lineage export` - (Preview) Returns complete lineage for the provided anchor node in CSV format.
- `aidp data-lineage fetch-entity-lineage` - (Preview) Returns lineage for a given entity object.
...
- `aidp volume upload-and-extract-volume-zip` - Creates or updates an asynchronous volume ZIP upload and extraction operation. Creates or updates an asynchronous volume ZIP upload and extraction operation. CREATE returns a PAR URL for uploading the ZIP bytes and an async operation key. UPDATE records the uploaded ZIP metadata so extraction can continue.
...
- `aidp volume zip-and-download-volume-folder` - Starts asynchronous creation of a ZIP archive for a volume folder. Starts asynchronous creation of a ZIP archive for a volume folder. The response includes a PAR URL for downloading the archive after the operation succeeds.
...
- `aidp workflow list-task-run-retries` - Returns detailed information about retries of a task run with a given task run key.
...
- `aidp workspace-object upload-and-extract-workspace-zip` - Creates or updates an asynchronous workspace ZIP upload and extraction operation. Creates or updates an asynchronous workspace ZIP upload and extraction operation. CREATE returns a PAR URL for uploading the ZIP bytes and an async operation key. UPDATE records the uploaded ZIP metadata so extraction can continue.
...
- `aidp workspace-object zip-and-download-workspace-folder` - Starts asynchronous creation of a ZIP archive for a workspace folder. Starts asynchronous creation of a ZIP archive for a workspace folder. The response includes a PAR URL for downloading the archive after the operation succeeds.
